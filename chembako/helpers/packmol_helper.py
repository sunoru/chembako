...
class Packmol(CommandSet):
    ...
    def run(self, inp_str):
        sp = subprocess.Popen([self._packmol_bin], stdin=subprocess.PIPE, stdout=self._log_file, stderr=self._log_file)
        self._log_screen("Running packmol with pid: %s." % sp.pid)
        sp.stdin.write(inp_str)
        sp.stdin.close()
        sp.wait()
        self._log_screen("Packmol completed.")
